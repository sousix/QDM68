...
static void UpdateClientInfo( int clientNum, const char *info ) {
	if( !info || !info[0] ) {
		...
		Q_strncpyz( clientNames[clientNum], Info_ValueForKey( info, "n" ), sizeof( clientNames[0] ) );
                ...
                //Com_Printf(" -- %s \n", clientNames[clientNum] );
                //Com_Printf(" -- %s \n", info );
                //Com_Printf(" ------------ UpdateClientInfo ---------- \n");
                //Info_Print( info );
	}
}
...
char * GameStateParsed( void ) {
	int i;
        char *oneConfigString;
        char allConfigString[MAX_GAMESTATE_CHARS];
        allConfigString[0] = '\0';

	for( i=0 ; i<MAX_CONFIGSTRINGS ; i++ ) {
                oneConfigString = Com_GetStringFromGameState( &ds.gameState, i );
                if( oneConfigString[0] ) {
                        if( i < RESERVED_CONFIGSTRINGS ) {
				//Com_Printf( "%s Info:\n", (i == CS_SERVERINFO) ? "Server" : "System" );
                                //Info_Print( oneConfigString );
				//( "\n" );
                            if( oneConfigString[0] != '\\' && allConfigString[strlen(allConfigString)-1] != '\\' )
                                strcat( allConfigString, "\\" );
                            strcat( allConfigString, oneConfigString );
                        }/*else {
	...
	for( i=0 ; i<MAX_CLIENTS ; i++ ) {
                oneConfigString = Com_GetStringFromGameState( &ds.gameState, CS_PLAYERS + i );
                UpdateClientInfo( i, oneConfigString );
                if( oneConfigString[0] != '\\' && allConfigString[strlen(allConfigString)-1] != '\\' )
                    strcat( allConfigString, "\\" );
                strcat( allConfigString, oneConfigString );
            }
